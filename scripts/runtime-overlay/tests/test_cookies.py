...
class TestGetCookies:
    def test_prefers_saved_cookies_by_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda source, **_kwargs: ("chrome", {"a1": "fresh"}),
        )

        browser, cookies = get_cookies("webbridge")
    ...
    def test_force_refresh_bypasses_saved_cookies(self, monkeypatch):
        ...
        browser, cookies = get_cookies("webbridge", force_refresh=True)
        assert browser == "chrome"
        assert cookies == {"a1": "fresh"}
        assert saved == [{"a1": "fresh"}]

    def test_saved_source_never_refreshes_from_browser(self, monkeypatch):
        monkeypatch.setattr(
            "xhs_cli.cookies.load_saved_cookies",
            lambda: {"a1": "saved", "saved_at": 1},
        )
        monkeypatch.setattr(
            "xhs_cli.cookies.extract_browser_cookies",
            lambda *_args, **_kwargs: pytest.fail("saved source must not touch a browser"),
        )

        assert get_cookies("saved") == ("saved", {"a1": "saved"})


def test_account_alias_isolates_config_directory(monkeypatch, tmp_path):
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("XHS_ACCOUNT", "worker-a")

    config_dir = get_config_dir()

    assert config_dir == tmp_path / ".xiaohongshu-cli" / "accounts" / "worker-a"
    assert config_dir.stat().st_mode & 0o777 == 0o700


def test_webbridge_cookie_source(monkeypatch):
    monkeypatch.setenv("XHS_ACCOUNT", "worker-a")
    monkeypatch.setattr(
        "xhs_cli.webbridge.extract_xhs_cookies",
        lambda **kwargs: {"a1": kwargs["account"]},
    )

    assert extract_browser_cookies("webbridge") == ("webbridge", {"a1": "worker-a"})
