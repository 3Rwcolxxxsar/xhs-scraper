...
class XhsClient(
    ...
    def _rate_limit_delay(self) -> None:
        """Retain a small transport delay for callers that disable the safety guard."""
    ...
    def _handle_response(self, resp: httpx.Response) -> Any:
        if resp.status_code in (461, 471):
            ...
            logger.warning(
                "Captcha triggered (count=%d); pausing subsequent requests",
                self._verify_count,
            )
            self._risk.record_failure("verification_required", immediate_pause=True)
        ...
        code = data.get("code")
        if code == 300012:
            self._risk.record_failure("ip_blocked", immediate_pause=True)
            raise IpBlockedError()
        if code == 300015:
            raise SignatureError()
        if code == -100:
            self._risk.record_failure("session_expired", immediate_pause=True)
    ...
    def _request_with_retry(self, method: str, url: str, **kwargs) -> httpx.Response:
        self._risk.before_request(self._request_kind(url))
        if not self._risk.settings.enabled:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                if resp.status_code == 429:
                    self._risk.record_failure("rate_limited", immediate_pause=True)
