...
Usage:
    ...
    xhs search <keyword> [--sort SORT] [--time TIME] [--scope SCOPE] [--type TYPE] [--page N]
...
from .commands import auth, creator, interactions, notifications, reading, risk, social
...
@click.option(
    "--cookie-source",
    type=click.Choice(["webbridge", "saved"]),
    default="webbridge",
    show_default=True,
...
@click.option(
    ...
    default="",
    envvar="XHS_ACCOUNT",
    help="Local account alias; isolates cookies, caches, risk state, and run locks",
)
@click.pass_context
def cli(ctx, verbose: bool, cookie_source: str, account: str):
    """xhs — Xiaohongshu CLI via reverse-engineered API 📕"""
    from .cookies import normalize_account_name
...
cli.add_command(risk.risk_status)
cli.add_command(risk.risk_resume)
cli.add_command(risk.risk_pause)
