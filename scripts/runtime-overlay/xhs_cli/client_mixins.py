...
_SEARCH_NOTE_TYPE_TAGS = {0: "不限", 1: "视频笔记", 2: "普通笔记"}
_SEARCH_SORT_TAGS = {
    "general",
    "time_descending",
...
_SEARCH_TIME_TAGS = {"all": "不限", "day": "一天内", "week": "一周内", "half-year": "半年内"}
_SEARCH_RANGE_TAGS = {"all": "不限", "seen": "已看过", "unseen": "未看过", "followed": "已关注"}
_SEARCH_SESSION_TTL_SECONDS = 600
_SEARCH_SESSION_MAX_SIZE = 128
_SEARCH_SESSION_LOCK = threading.RLock()
_SEARCH_SESSION_CACHE: OrderedDict[tuple[str, str, int, str, str], dict[str, Any]] = OrderedDict()
...
def _search_session_key(
    ...
    return (keyword.strip(), sort, note_type, note_time, note_range)
...
def _serialize_search_session_key(key: tuple[str, str, int, str, str]) -> str:
    return json.dumps(list(key), ensure_ascii=False)


def _deserialize_search_session_key(value: str) -> tuple[str, str, int, str, str] | None:
    try:
        parts = json.loads(value)
    except (TypeError, ValueError, json.JSONDecodeError):
        return None
    if not isinstance(parts, list) or len(parts) not in {3, 5}:
        return None
    keyword, sort, note_type = parts[:3]
    note_time, note_range = parts[3:] if len(parts) == 5 else ("all", "all")
    ...
    try:
        ...
        return None
    if note_time not in _SEARCH_TIME_TAGS or note_range not in _SEARCH_RANGE_TAGS:
        return None
    return (keyword, sort, normalized_note_type, note_time, note_range)


def _load_search_session_cache_from_disk(
    path: Path,
) -> OrderedDict[tuple[str, str, int, str, str], dict[str, Any]]:
    if not path.exists():
        return OrderedDict()
    try:
    ...
    normalized: list[tuple[tuple[str, str, int, str, str], dict[str, Any]]] = []
...
def _acquire_search_session(
    ...
    now = time.time()
    key = _search_session_key(keyword, sort, note_type, note_time, note_range)
...
def get_search_session_stats() -> dict[str, Any]:
    ...
    with _SEARCH_SESSION_LOCK:
        ...
        return {
            ...
            "last_note_time": last_key[3],
            "last_note_range": last_key[4],
        }
...
class ReadingEndpointsMixin:
    ...
    def search_notes(
        ...
        if sort not in _SEARCH_SORT_TAGS:
            raise ValueError(f"Unsupported search sort: {sort}")
        if note_type not in _SEARCH_NOTE_TYPE_TAGS:
            raise ValueError(f"Unsupported note type: {note_type}")
        if note_time not in _SEARCH_TIME_TAGS:
            raise ValueError(f"Unsupported note time: {note_time}")
        if note_range not in _SEARCH_RANGE_TAGS:
            raise ValueError(f"Unsupported note range: {note_range}")
        ...
        result = self._main_api_post("/api/sns/web/v1/search/notes", {
            ...
            "filters": [
                ...
                {"tags": [_SEARCH_NOTE_TYPE_TAGS[note_type]], "type": "filter_note_type"},
                {"tags": [_SEARCH_TIME_TAGS[note_time]], "type": "filter_note_time"},
                {"tags": [_SEARCH_RANGE_TAGS[note_range]], "type": "filter_note_range"},
    ...
    def get_all_comments(
        ...
        while pages < max_pages:
            ...
            if not isinstance(data, dict):
                stopped_reason = "invalid_response"
                break
        ...
        result = {
            ...
            "has_more": has_more,
            "cursor": cursor if has_more else "",
        ...
        if include_replies:
            ...
            reply_failures: list[dict[str, str]] = []
            for comment in all_comments:
                root_comment_id = str(comment.get("id") or comment.get("comment_id") or "")
                reply_count = comment.get("sub_comment_count", 0)
                if not root_comment_id or not reply_count:
                    continue
                try:
                    page = self.get_all_sub_comments(
                        note_id,
                        root_comment_id,
                        xsec_token=xsec_token,
                    ...
                    if isinstance(
                        exc,
                        (NeedVerifyError, RateLimitedError, RiskPausedError, SessionExpiredError, IpBlockedError),
                    ):
                        raise
                    reply_failures.append({"root_comment_id": root_comment_id, "reason": str(exc)})
                    continue
                for reply in page["comments"]:
                    if isinstance(reply, dict):
                        replies.append({**reply, "_root_comment_id": root_comment_id})
            result["replies"] = replies
            result["reply_failures"] = reply_failures
        return result
    ...
    def get_sub_comments(
        ...
        return self._main_api_get("/api/sns/web/v2/comment/sub/page", params)

    def get_all_sub_comments(
        self,
        note_id: str,
        root_comment_id: str,
        cursor: str = "",
