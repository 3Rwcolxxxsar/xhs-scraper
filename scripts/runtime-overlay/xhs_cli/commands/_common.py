...
from ..risk_control import RunLock
...
def _cookie_source(ctx) -> str:
    return ctx.obj.get("cookie_source", "webbridge") if ctx.obj else "webbridge"
    ...
    *,
    ...
    if isinstance(exc, (NoCookieError, SessionExpiredError)):
        details = {
            ...
            "command": "xhs --cookie-source webbridge login --qrcode",
            "message": "请在 Kimi WebBridge 打开的真实浏览器中登录，然后重新执行原命令。",
        }
