...
class TestCliBasic:
    ...
    def test_global_help_lists_account_and_webbridge(self):
        result = runner.invoke(cli, ["--help"])
        assert result.exit_code == 0
        assert "--account" in result.output
        assert "webbridge" in result.output.lower()

    def test_direct_browser_profile_option_is_not_supported(self):
        result = runner.invoke(cli, ["--browser-profile", "/tmp/Profile 1", "status"])
    ...
    def test_all_commands_registered(self):
        ...
        commands_expected = [
            ...
            "risk-status", "risk-resume", "risk-pause",
            # Reading
    ...
    def test_read_error_yaml_when_not_logged_in(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda source, force_refresh=False, **_kwargs: (_ for _ in ()).throw(NoCookieError(source)),
        )
        ...
        assert payload["error"]["details"]["action_required"] == "scan_qr"
        assert payload["error"]["details"]["command"] == "xhs --cookie-source webbridge login --qrcode"
    ...
    def test_status_rejects_guest_profile(self, monkeypatch):
        monkeypatch.setenv("OUTPUT", "auto")
        monkeypatch.setattr(
            "xhs_cli.commands.auth.run_client_action",
            lambda ctx, action: {"guest": True, "nickname": "Unknown"},
        )
    ...
    def test_search_forwards_sort_time_and_scope(self, monkeypatch):
        captured = {}
        ...
        monkeypatch.setattr(
            "xhs_cli.commands.reading.run_client_action",
            lambda _ctx, action: action(FakeClient()),
        )
        ...
        assert result.exit_code == 0
        assert captured["sort"] == "collect_descending"
        assert captured["note_time"] == "half-year"
        assert captured["note_range"] == "followed"
    ...
    def test_read_index_resolves_note_context(self, monkeypatch):
        ...
        monkeypatch.setattr("xhs_cli.commands.reading.run_client_action", lambda ctx, action: action(FakeClient()))
    ...
    def test_comments_normalized_json_has_capture_schema(self, monkeypatch):
        monkeypatch.setattr(
            "xhs_cli.commands.reading.run_client_action",
            lambda _ctx, _action: {
                "comments": [{
                    "id": "comment-1",
                    "content": "hello",
                    "user_info": {"user_id": "user-1", "nickname": "tester"},
                }],
                "has_more": False,
            },
        )

        result = runner.invoke(cli, ["comments", "note-123", "--normalized", "--json"])

        assert result.exit_code == 0
        payload = yaml.safe_load(result.output)
        assert payload["data"]["schema"] == "xhs-comment-capture.v1"
        assert payload["data"]["comments"][0]["comment_id"] == "comment-1"

    def test_sub_comments_index_and_token_support_all_pages(self, monkeypatch):
        monkeypatch.setattr(
            "xhs_cli.note_refs.get_note_by_index",
            lambda idx: {
                "note_id": "note-abc",
        ...
        class FakeClient:
            def get_all_sub_comments(self, note_id, comment_id, **kwargs):
                called.update({"note_id": note_id, "comment_id": comment_id, **kwargs})
                return {"comments": [], "total_fetched": 0, "pages_fetched": 1}

        monkeypatch.setattr("xhs_cli.commands.reading.run_client_action", lambda ctx, action: action(FakeClient()))
        result = runner.invoke(cli, ["sub-comments", "1", "root-1", "--all", "--yaml"])

        assert result.exit_code == 0
        assert called["note_id"] == "note-abc"
        assert called["comment_id"] == "root-1"
        assert called["xsec_token"] == "token-abc"
