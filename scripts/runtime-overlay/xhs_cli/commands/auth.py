...
from ..cookies import clear_cookies, get_cookies, save_cookies
...
@click.option("--qrcode", "use_qrcode", is_flag=True, default=False,
              help="Open the Kimi WebBridge browser and wait for QR login")
@click.pass_context
def login(ctx, cookie_source: str | None, as_json: bool, as_yaml: bool, use_qrcode: bool):
    ...
    if use_qrcode:
        def _login_with_qrcode() -> None:
            from ..webbridge import wait_for_xhs_login

            account = ctx.obj.get("account", "") if ctx.obj else ""
            cookies = wait_for_xhs_login(account=account)
        ...
        handle_errors(
            ...
            as_json=as_json,
            as_yaml=as_yaml,
            prefix="Kimi WebBridge login failed",
        )
        return

    # Browser cookie extraction (default)
    if cookie_source is None:
        cookie_source = ctx.obj.get("cookie_source", "webbridge") if ctx.obj else "webbridge"
